...
function LabelInput({ 
  ...
  // Label classes
  const combinedLabelClasses = `${labelSize} ${labelWeight} ${labelColor} ${labelClassName}`.trim()
  ...
  if (labelPosition === 'top') {
    return (
      <div className="w-full">
        {label && (
          <label htmlFor={id} className={`block mb-2 ${combinedLabelClasses}`}>
            {label}
  ...
  // Render label on left (horizontal layout)
  return (
    <div className="flex items-center gap-4 w-full">
      {label && (
        <label htmlFor={id} className={`${combinedLabelClasses} whitespace-nowrap`}>
          {label}
          {required && <span className="text-red-500 ml-1">*</span>}
        </label>
      )}
